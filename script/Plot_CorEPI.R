# Title     : correlate EPI among diferent strains
# Objective :
# Created by: yiquan
# Created on: 6/7/21
...
#merge df
com_epi_df <- bind_cols(list(HK68_epi,Bk79_epi,Bei89_epi,Mos99_epi,Vic11_epi,HK19_epi)) %>%
...
ggsave('graph/compare_EPI.png',bg='white',p,width = 4, height = 4)
...
#merge df
com_add_df <- bind_cols(list(HK68_add,Bk79_add,Bei89_add,Mos99_add,Vic11_add,HK19_add)) %>%
...
p2  <- ggpairs(com_add_df,columns=3:8,lower =list(continuous=wrap(ggally_points,size=0.1,alpha=1)),
  ...
  scale_x_continuous(n.breaks = 3.5) +
  scale_y_continuous(n.breaks = 4)
ggsave('graph/compare_ADD.png',bg='white',p2,width = 4, height = 4)
...
heat_p <- heat_p+
	...
	theme(
		axis.title.x = element_blank(),
  		...
  		axis.ticks = element_blank())
ggsave('graph/heatmap_epi_add_cor.png',bg='white',heat_p,width = 2.3, height = 1.9)