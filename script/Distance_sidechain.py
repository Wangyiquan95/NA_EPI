...
def Distance(resi_ls,output):
    ...
    for resi1 in resi_ls:
        for resi2 in resi_ls:
            ...
            pair1=resi1.rsplit('_')[1]
            pair2=resi2.rsplit('_')[1]
            pair=pair1+'-'+pair2
            outfile.write(str(pair) + "\t" + str(dst) + "\n")
    outfile.close()

def main():
    cmd.load('script/2aep.cif', 'N2')
    resi_ls = ['resi 328 and (not name c+n+o)', 'resi 329 and (not name c+n+o)', 'resi 344 and (not name c+n+o)', 'resi 367 and (not name c+n+o)',
               'resi 368 and (not name c+n+o)', 'resi 369 and (not name c+n+o)', 'resi 370 and (not name c+n+o)']
    output = 'result/SC_COG_distance.tsv'
    cog_ls=[]
    for resi in resi_ls:
        resi_name=cmd.get_legal_name(resi)+'_COM'
        cog_ls.append(resi_name)
        com(resi, state=1)

    Distance(cog_ls,output)
